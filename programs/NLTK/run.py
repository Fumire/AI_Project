...
speeches = ["""In this grave hour, perhaps the most fateful in our history, I send to every household of my peoples, both at home and overseas, this message, spoken with the same depth of feeling for each one of you as if I were able to cross your threshold and speak to you myself.
...
for speech in speeches:
    ...
    for sentence in nltk.sent_tokenize(speech):
        ...
        for subtree in tree:
            ...
            if phrase in [",", ";"]:
                pydub.AudioSegment.silent(duration=300).export("/tmp/" + str(mp3_count) + ".wav", format="wav")
            elif phrase in [".", ":"]:
                pydub.AudioSegment.silent(duration=600).export("/tmp/" + str(mp3_count) + ".wav", format="wav")
            else:
                text_to_mp3(phrase, "/tmp/" + str(mp3_count) + ".wav")
            mp3_count += 1

    for i in range(mp3_count):
        print(i, "/", mp3_count)
        tmp_file = pydub.AudioSegment.from_file("/tmp/" + str(i) + ".wav").fade_in(200).fade_out(200)
        mp3_file += tmp_file
        os.remove("/tmp/" + str(i) + ".wav")
